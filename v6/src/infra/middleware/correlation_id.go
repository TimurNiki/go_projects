...
func AddCorrelationId(app *fiber.App) fiber.Router {
	return app.Use("/orders", func(ctx *fiber.Ctx) error {
		...
		if correlationId == "" {
			return ctx.Status(fiber.StatusBadRequest).JSON("Missing x-correlationid header")
		}

		_, err := uuid.Parse(correlationId)
		if err != nil {
			return ctx.Status(fiber.StatusBadRequest).JSON("Invalid x-correlationid header")
